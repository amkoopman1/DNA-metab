...
setwd("C:/Users/abelk/Documents/Conservation MSc/DNA metabarcoding RSP1/Sequencing data/habrok_download/primer_test_may_sep/primer_test")
...
blast.files <- tibble(
  file = list.files(
    ...
    recursive = TRUE, 
    full.names = TRUE)) |> 
  mutate(reference_db = str_extract(file, "gb|bold"), locus = str_extract(file, "jusino|verkuil|leray"))
...
## ?? many rows lose their bitscore/eval/identity etc!
## why full join again with the unfiltered taxa.tophit?
...
taxa.tophit |> 
  mutate(identity.max.dif = round(identity.max, 1) - identity.max.lca,
         ...
         n.dif = n - n.lca) |> 
  filter(identity.max.dif !=0 | identity.min.dif != 0 | n.dif != 0) %>% View()
...
length(unique(taxa$asv_id)) # 837 ASVs assigned by genbank, bold or both
...
# 2 ASVs have been identified in this way
...
# 176 ASVs have been identified in this way
...
# 126 ASVs have been identified in this way
...
# 61 ASVs have been identified in this way
...
# 23 ASVs have been identified in this way
...
# 7 ASVs have been identified in this way
...
# 26 ASVs have been identified in this way
...
# 11 ASVs have been identified in this way
...
# 245 ASVs have been identified in this way
...
### Export
write.csv(taxatable, file = "07-phyloseq/taxatable_20260505.csv", quote = FALSE, row.names = FALSE)
...
# Export
write.csv(taxatable.nsr.nourl, file = "07-phyloseq/taxatable_nsr_20260505.csv", quote = FALSE, row.names = FALSE)
